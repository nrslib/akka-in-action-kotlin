        ...
        return post {
            entity(Jackson.unmarshaller(objectMapper, EventDescription::class.java)) { eventDescription ->
                ...
                onSuccess(createEvent(event, ed.tickets)) {
                    when (it) {
                        is BoxOffice.Companion.EventCreated -> complete(
                            StatusCodes.CREATED,
                            it.event,
                            Jackson.marshaller()
                        )
...
interface BoxOfficeApi {
    ...
    fun createEvent(event: String, nrOrTickets: Int) =
        AskPattern.ask(
            boxOffice,
            { replyTo: ActorRef<BoxOffice.Companion.EventResponse> ->
                BoxOffice.Companion.CreateEvent(
                    event,
                    nrOrTickets,
                    replyTo
                )
    ...
    fun getEvents() =
        AskPattern.ask(
            boxOffice,
            { replyTo: ActorRef<BoxOffice.Companion.Events> -> BoxOffice.Companion.GetEvents(replyTo) },
            timeout,
            context.system().scheduler()
        )
    ...
    fun getEvent(event: String) =
        AskPattern.ask(
            boxOffice,
            { replyTo: ActorRef<Optional<BoxOffice.Companion.Event>> -> BoxOffice.Companion.GetEvent(event, replyTo) },
            timeout,
            context.system().scheduler()
        )
    ...
    fun cancelEvent(event: String) =
        AskPattern.ask(
            boxOffice,
            { replyTo: ActorRef<Optional<BoxOffice.Companion.Event>> ->
                BoxOffice.Companion.CancelEvent(
                    event,
                    replyTo
                )
            },
    ...
    fun requestTickets(event: String, tickets: Int) =
        AskPattern.ask(
            boxOffice,
            { replyTo: ActorRef<TicketSeller.Companion.Tickets> ->
                BoxOffice.Companion.GetTickets(
                    event,
                    tickets,
                    replyTo
                )
